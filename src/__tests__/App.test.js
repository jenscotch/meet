...
describe('App /> integration', () => {
    ...
    test('get list of events matching the city selected by the user', async () => {
        ...
        const suggestions = CitySearchWrapper.find('suggestions');
    ...
    test('get list of all events when user selects "See all cities"', async () => {
        ...
        //await suggestionItems.at(suggestionItems.length - 1);
        const allEvents = await getEvents();
    ...
    test('get list of events that matches the number of events selected by user', async () => {
       ...
       //await NumberOfEventsWrapper.instance().handleInputChanged();
        //expect(AppWrapper.state('eventCount')).toEqual(NumberOfEventsWrapper);
        //expect(AppWrapper.state('events').length).toBe(NumberOfEventsWrapper);
        AppWrapper.unmount();
    });
});